...
import { encoder, focusWrapTarget, highlight, isSearchShortcut } from "./search.inline"
...
describe("search result highlighting", () => {
  test("treats regular-expression metacharacters as literal text", () => {
    assert.strictEqual(
      highlight("[.*]", "Use [.*] literally"),
      'Use <span class="highlight">[.*]</span> literally',
    )
  })

  test("escapes metadata-looking markup before DOM rendering", () => {
    const title = '<img src=x onerror="alert(1)">'
    assert.strictEqual(
      highlight("img", title),
      '&lt;<span class="highlight">img</span> src=x onerror=&quot;alert(1)&quot;&gt;',
    )
  })
})

describe("search keyboard behavior", () => {
  const shortcut = (overrides: Partial<Parameters<typeof isSearchShortcut>[0]> = {}) => ({
    altKey: false,
    ctrlKey: true,
    key: "k",
    metaKey: false,
  ...
  test("opens only for the unshifted Ctrl/Cmd+K shortcut", () => {
    assert.strictEqual(isSearchShortcut(shortcut()), true)
    assert.strictEqual(
      isSearchShortcut(shortcut({ ctrlKey: false, metaKey: true, key: "K" })),
      true,
    )
    assert.strictEqual(isSearchShortcut(shortcut({ shiftKey: true })), false)
    assert.strictEqual(isSearchShortcut(shortcut({ altKey: true })), false)
    assert.strictEqual(isSearchShortcut(shortcut({ repeat: true })), false)
  })

  test("lets Tab reach scope and close controls before wrapping", () => {
    const controls = ["query", "answers", "archive", "close", "result"]
    assert.strictEqual(focusWrapTarget(controls, "query", false), undefined)
    assert.strictEqual(focusWrapTarget(controls, "answers", false), undefined)
    assert.strictEqual(focusWrapTarget(controls, "archive", false), undefined)
    assert.strictEqual(focusWrapTarget(controls, "result", false), "query")
    assert.strictEqual(focusWrapTarget(controls, "query", true), "result")
    assert.strictEqual(focusWrapTarget(controls, "outside", false), "query")
  })
})
